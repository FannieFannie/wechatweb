...
export default {
  ...
  async created () {
    ...
    if (that.$route.query.data) {
      that.title = that.$route.query.data.title ? that.$route.query.data.title : '申报记录详情'
      ...
      that.submitLast().then(function (result) {
        ...
        if (result == true) {
          that.$weui.toast("提交申报成功", {
            duration: 1000,
            className: "bears",
            callback: function () { that.$router.push({ name: 'repairLog' }) }
          });
